// ============================================================
//  InventoryPage  –  Inventory feature (ice cream theme)
// ============================================================
...
const InventoryModal: React.FC<InventoryModalProps> = ({
  open, onClose, item, customCols, onSaved, farmId, threshold,
}) => {
  ...
  useEffect(() => {
    if (item) {
      ...
      setForm({ product_id: "", name: "", quantity: 0, price: 0, date_harvested: "", custom_fields: {} });
    }
  }, [item, open]);
    ...
    <Modal
      ...
      footer={
        <div className={styles.modalFooter}>
          <Button variant="cancel" type="button" onClick={onClose} data-testid="btn-cancel-inv">
            Cancel
          </Button>
          <Button variant="success" type="submit" form="inventory-form" loading={saving} data-testid="btn-save-inv">
            Save
          </Button>
        </div>
      }
    >
      <form id="inventory-form" onSubmit={handleSubmit} data-testid="inventory-form">
        <div className={styles.formRow}>
          <div className={styles.formGroup}>
            <label htmlFor="field-product-id">Product ID *</label>
            <input
              id="field-product-id"
        ...
        <div className={styles.formGroup}>
          <label htmlFor="field-date">Production Date</label>
          <input
            id="field-date"
            type="date"
        ...
        {customCols.map(col => (
          <div className={styles.formGroup} key={col.key}>
            <label htmlFor={`field-custom-${col.key}`}>{col.name}</label>
            <input
              id={`field-custom-${col.key}`}
              value={form.custom_fields?.[col.key] ?? ""}
              onChange={e => set("custom_fields", { ...form.custom_fields, [col.key]: e.target.value })}
...
export const InventoryPage: React.FC = () => {
  ...
  const { data: allItems = [], isLoading } = useQuery({
    ...
    queryFn:  () =>
      inventoryService.list(farm!.id, { search, status: statusFilter }).then(r => r.data ?? []),
    enabled: !!farm?.id,
  });

  const { sort, sorted, column: sortCol, direction: sortDir } = useSort<InventoryItem>("name");
  ...
  const COLUMNS: Column<InventoryItem>[] = [
    ...
    { key: "price",        header: "Price (₱)",    sortable: true, render: r => fmt(r.price) },
    { key: "total_value",  header: "Total Value",  sortable: true, render: r => fmt(r.total_value) },
    { key: "date_harvested", header: " Production Date",  sortable: true },
    {
      key: "status",
      header: "Status",
      ...
      render: r => (
        <div className={styles.actions}>
          <Button
            ...
            icon={<i className="fa-solid fa-pencil" aria-hidden />}
            onClick={() => openEdit(r)}
          ...
          <Button
            ...
            onClick={() => { sel.clear(); sel.toggle(r.id); setDeleteOpen(true); }}
  ...
  return (
    <section className={styles.page} data-testid="inventory-page">
      ...
      <DataTable<InventoryItem>
        ...
        loading={isLoading}
        emptyMessage="No inventory items yet — add your first scoop! 🍦"
      ...
      <Modal
        ...
        footer={
          <>
            <Button variant="cancel" onClick={() => setDeleteOpen(false)} data-testid="btn-cancel-delete">
              Cancel
            </Button>
            <Button variant="danger" onClick={handleDeleteConfirm} loading={deleting} data-testid="btn-confirm-delete">
              Delete
            </Button>
          </>
        }
